...
head:
  - - meta
    - name: description
      content: Guide to Using Cherry Studio via AGIPower
  - - meta
    - name: keywords
...
AGIPower fully supports the OpenAI Chat Completions protocol and can be seamlessly integrated into clients like Cherry Studio that support custom OpenAI-compatible endpoints. Simply add AGIPower as a model provider to use all models that support the OpenAI protocol.

Note that the base_url for the OpenAI protocol is `https://api.agipower.ai/v1/`.
</Info>
...
![Add AGIPower Provider](https://github.com/user-attachments/assets/fee04a31-9f24-4c42-9ba6-6f0e1df03f92)
...
Make sure to replace `sk-xxx` with your actual AGIPower API Key. You can obtain your API Key from the [AGIPower Console](https://agipower.ai/settings/keys).
</Warning>
...
Go to the chat interface, select any model under the AGIPower provider, and start using it.
...
- Visit the [AGIPower Model List](https://agipower.ai/models) and filter by "OpenAI API Compatible"
- Or visit the detail page of a specific model to check its protocol support

</Info>

<Tip title="Explore More Models">
Visit the [AGIPower Model List](https://agipower.ai/models) to learn about all available models and their details, including pricing, context length, feature capabilities, and more.
</Tip>
...
- Log in to the [AGIPower Console](https://agipower.ai/settings/keys) to check the API Key status
...
- **Business Contact Email**: [support@agipower.ai](mailto:support@agipower.ai)
