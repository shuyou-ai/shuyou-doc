...
head:
  - - meta
    - name: description
      content: Guide to Using Claude Code CLI with ShuYou
  - - meta
    - name: keywords
      content: ShuYou, best practices, integration, claude, code, Anthropic, Claude, GPT, API
---

# Guide to Using Claude Code CLI with ShuYou

<Info title="Compatibility Notes">

Claude Code is Anthropic’s official Coding Agent. With ShuYou integration, you can access a broader set of model options instead of being limited to Anthropic’s official Claude models.

For example, via ShuYou you can use GPT-5.2 series, Claude-4.5 series, Gemini-3 series, Grok 4.1 series, Doubao-Seed-Code, Kimi-K2, Minimax-M2, GLM-4.6, DeepSeek-V3.2, Qwen3-Coder-Plus, and more in Claude Code. For the full list of supported models, see the [official model list](https://shuyou.ai/models?sort=newest&supported_protocol=messages).

ShuYou fully supports the Anthropic API protocol and can be seamlessly integrated into tools like Claude Code and Cursor. You only need to change two parameters to use it.

Note: the Anthropic-compatible `base_url="https://shuyou.ai"`.
</Info>
...
By default, Claude Code connects directly to Anthropic’s official service. However, by configuring environment variables, we can redirect its requests to ShuYou. This provides several benefits:

- **No changes to Claude Code itself**: Switch endpoints using only environment variables
- **Authenticate with a ShuYou API Key**: Replaces Anthropic’s official API key
- **More model choices**: In addition to Claude, you can use GPT, Gemini, Qwen, and more

The core configuration is to set two key environment variables: `ANTHROPIC_BASE_URL` (the ShuYou endpoint) and `ANTHROPIC_AUTH_TOKEN` (your ShuYou API Key), so that all Claude Code requests are proxied through ShuYou.
...
Therefore, when connecting to ShuYou, it’s recommended to standardize on **shell profile environment variables** to ensure both login and requests go through ShuYou’s Anthropic-compatible endpoint.
</Warning>

### Step 0: Get a ShuYou API Key

Before configuring Claude Code, you need a ShuYou API Key. ShuYou offers two billing options—choose based on your use case:
...
This step writes the ShuYou connection configuration into your shell config file so it automatically applies whenever you open a terminal.
...
# Connect Claude Code to ShuYou instead of Anthropic’s official service

# Core settings: ShuYou endpoint and authentication
export ANTHROPIC_BASE_URL="https://shuyou.ai"  # ShuYou Anthropic-compatible endpoint
export ANTHROPIC_AUTH_TOKEN="sk-ss-v1-xxx"                   # Replace with your ShuYou API Key (subscription sk-ss-v1-xxx or pay-as-you-go sk-xxx)
...
# Core settings: ShuYou endpoint and authentication
$env:ANTHROPIC_BASE_URL = "https://shuyou.ai"  # ShuYou Anthropic-compatible endpoint
$env:ANTHROPIC_AUTH_TOKEN = "sk-ss-v1-xxx"                   # Replace with your ShuYou API Key (subscription sk-ss-v1-xxx or pay-as-you-go sk-xxx)
...
Be sure to replace `sk-ss-v1-xxx` or `sk-xxx` in the configuration with your real ShuYou API Key:

**Subscription API Key (recommended for personal development)**

- Format: `sk-ss-v1-xxx`
- Get it from: [Subscription management page](https://shuyou.ai/platform/subscription)
- Detailed guide: [Subscription plan docs](/en/guide/subscription)

**Pay-as-you-go API Key (production)**

- Format: `sk-xxx`
- Get it from: [Pay-as-you-go page](https://shuyou.ai/platform/pay-as-you-go)
...
| ------------------------------------------ | ------------------ | ---------------------------------------------------------------- |
| `ANTHROPIC_BASE_URL`                       | Service endpoint   | Redirects Claude Code requests to ShuYou                         |
| `ANTHROPIC_AUTH_TOKEN`                     | Auth token         | Your ShuYou API Key (subscription or pay-as-you-go)              |
...
After the environment variables are configured, you can start Claude Code. On first launch, it will automatically authenticate via ShuYou.
...
4. On first launch, Claude Code will:
   ...
   - Authenticate via the ShuYou endpoint specified by `ANTHROPIC_BASE_URL`
...
After launching successfully, it’s recommended to verify that Claude Code is correctly connected to ShuYou.
...
Anthropic base URL: https://shuyou.ai  # [!code highlight]
```

**What to verify:**

- ✅ `Auth token` should display `ANTHROPIC_AUTH_TOKEN` (meaning it’s read from environment variables)
- ✅ `Anthropic base URL` should display `https://shuyou.ai` (the ShuYou endpoint)

If everything matches, your setup is complete—you can now use Claude Code through ShuYou.
...
Models compatible with the Anthropic protocol are being adapted in batches. You can filter currently supported models via the [official model list](https://shuyou.ai/models?sort=newest&supported_protocol=messages) and select the Anthropic Messages protocol:
...
&lt;div style="text-align: center;"&gt;
  <img src="https://cdn.marmot-cloud.com/storage/ShuYou/2026/02/03/S4ThMGS/cc-vs.png" alt="Install Claude Code Extension" />
&lt;/div&gt;
...
&lt;div style="text-align: center;"&gt;
  <img src="https://cdn.marmot-cloud.com/storage/ShuYou/2026/02/03/lDKDS2r/cc-vs2.png" alt="Open Claude Code settings" />
&lt;/div&gt;
...
&lt;div style="text-align: center;"&gt;
  <img src="https://cdn.marmot-cloud.com/storage/ShuYou/2026/02/03/w8EeQzh/cc-vs3.png" alt="Configure model and environment variables" />
&lt;/div&gt;
...
1. **Replace the API Key**: Replace `sk-ss-v1-xxx` with your real ShuYou API Key
...
After setup, you can use multiple ShuYou models in Claude Code:
...
2. **Validate the API Key**:
   ...
   - Pay-as-you-go: visit the [pay-as-you-go page](https://shuyou.ai/platform/pay-as-you-go) to confirm sufficient balance
...
#### Authentication Failure When Switching from Another Platform to ShuYou
**Issue**: You previously used a Claude Code official account or another platform (e.g., MiniMax, GLM, etc.). After switching to ShuYou, authentication fails or configuration conflicts occur.

**Solution**:

This usually happens because old config files cached prior authentication info, causing conflicts with the new ShuYou configuration. Clean up and reconfigure as follows:
...
2. **Confirm your shell environment variables are configured correctly**:
   ...
   Check whether your shell config file (`~/.zshrc` or `~/.bashrc`) contains the full ShuYou configuration. Refer to the [Step 1: Configure Shell Environment Variables](#step-1-configure-shell-environment-variables-recommended) section above, and ensure it includes these key variables:

   ```bash
   export ANTHROPIC_BASE_URL="https://shuyou.ai"
   export ANTHROPIC_AUTH_TOKEN="sk-ss-v1-xxx"  # Replace with your ShuYou API Key
...
4. **Verify environment variables are applied**:
   ...
   ```bash [macOS/Linux]
   # Check ShuYou endpoint
   echo $ANTHROPIC_BASE_URL
   # Should output: https://shuyou.ai

   # Check whether the API Key is set
   echo $ANTHROPIC_AUTH_TOKEN
   # Should output your ShuYou API Key
...
6. **Verify the connection**:
   ...
   Anthropic base URL: https://shuyou.ai
   ```
...
- Use the [ShuYou model list](https://shuyou.ai/models) and filter by "Anthropic API Compatible" to see currently supported models
- Or open the model’s detail page to confirm Anthropic protocol support
- Choose a model from the supported list above


#### Connection Failures
**Issue**: Claude Code cannot connect to the ShuYou service.

**Solution**:

- Check that your network connection is working
- Verify `ANTHROPIC_BASE_URL` is set to `https://shuyou.ai`
...
2. **Check the model configuration**:
   ...
   - Confirm the model supports the Anthropic protocol (check via the [model list](https://shuyou.ai/models?sort=newest&supported_protocol=messages))
...
See the [ShuYou model list](https://shuyou.ai/models) for all available models and detailed information.
</Info>
...
- **Support**: [support@shuyou.ai](mailto:support@shuyou.ai)
- **Business**: [support@shuyou.ai](mailto:support@shuyou.ai)
