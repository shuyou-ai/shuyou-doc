...
head:
  - - meta
    - name: description
      content: Guide to Using Gemini CLI with ShuYou
  - - meta
    - name: keywords
      content: ShuYou, best practices, integration, Gemini CLI, Google, AI, terminal, agent
---

# Guide to Using Gemini CLI with ShuYou

Gemini CLI is an open-source AI terminal agent tool released by Google that brings Gemini’s capabilities directly into your terminal. Built on the ReAct (Reason + Act) loop architecture, it can autonomously handle complex tasks such as coding, debugging, file operations, and content generation. Gemini CLI offers a 1M-token context window, a request rate limit of 60 requests/minute, and rich MCP tool integration. With ShuYou, you can configure a custom API endpoint for Gemini CLI, enabling more flexible model selection and a more stable service experience.

## Prerequisites

- Node.js 18+
- ShuYou API Key (see [Get a ShuYou API Key](#get-a-ShuYou-api-key) below)
...
How to get:
1. Visit https://shuyou.ai/platform/subscription
2. Choose a plan
...
How to get:
1. Visit https://shuyou.ai/platform/pay-as-you-go
...
GOOGLE_GEMINI_BASE_URL=https://api.shuyou.ai  # [!code highlight]
```
...
export GOOGLE_GEMINI_BASE_URL="https://api.shuyou.ai"  # [!code highlight]
```
...
$env:GOOGLE_GEMINI_BASE_URL = "https://api.shuyou.ai"  # [!code highlight]
...
Be sure to replace `sk-ss-v1-xxx` with your real ShuYou API Key. You can get your API Key in the [ShuYou Console](https://shuyou.ai/settings/keys).
</Warning>
...
- `GEMINI_API_KEY`: your ShuYou API Key
- `GEMINI_MODEL`: the model to use (any ShuYou-supported Google model)
- `GOOGLE_GEMINI_BASE_URL`: ShuYou Vertex AI–compatible endpoint
...
If it can read the file and return a summary, ShuYou + Gemini CLI read-only analysis is working.
...
**When using ShuYou**, choose the API Key method and set your ShuYou API Key in the `GEMINI_API_KEY` environment variable.
...
With ShuYou, you can use multiple Gemini models in Gemini CLI. Use the `GEMINI_MODEL` environment variable to specify a model:
...
- View available Google AI protocol models via the [ShuYou model list](https://shuyou.ai/models?sort=newest&supported_protocol=google)
...
Even if a model name switch works, it does not guarantee tool calling will work as well. Current limitations mainly come from ShuYou’s compatibility with the Gemini CLI Agent protocol, not just the model itself.
</Tip>
...
- Once ShuYou completes function calling compatibility, this will be supported automatically and you won’t need this modification
</Tip>
...
- Verify the key status in the [ShuYou Console](https://shuyou.ai/settings/keys)


#### Connection failure
**Issue**: Gemini CLI cannot connect to ShuYou.

**Solution**:

- Check your network connection
- Verify the Base URL is set to `https://api.shuyou.ai`
- Check whether firewall settings are blocking outbound connections
- Try `curl https://api.shuyou.ai/models` to test connectivity
...
- Once ShuYou completes function calling compatibility, this will be supported automatically
...
- Check availability via the [ShuYou model list](https://shuyou.ai/models?sort=newest&supported_protocol=google)
...
export GOOGLE_GEMINI_BASE_URL="https://api.shuyou.ai"
