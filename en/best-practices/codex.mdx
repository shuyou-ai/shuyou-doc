...
head:
  - - meta
    - name: description
      content: Guide to using Codex CLI with ShuYou
  - - meta
    - name: keywords
      content: ShuYou, best practices, integration, codex, OpenAI, API
---

# Guide to Using Codex CLI with ShuYou

Codex CLI is an open-source programming assistant from OpenAI that runs in your local terminal. It can read, modify, and run code within a directory you choose. Built with Rust, it is fast and efficient, and is continuously improved on GitHub. By integrating with ShuYou, you gain access to more model options instead of being limited to the official OpenAI API.
...
Note the OpenAI-compatible setting: base_url="https://api.shuyou.ai/v1".
</Info>
...
Add your ShuYou API Key to your shell configuration file:

```bash
# Edit ~/.zshrc or ~/.bashrc depending on the terminal you use
export ShuYou_API_KEY="sk-xxx"  # [!code highlight]
```

<Warning title="Important">
Make sure to replace `sk-xxx` with your actual ShuYou API Key. You can obtain an API Key in the [ShuYou Console](https://shuyou.ai/settings/keys).
</Warning>
...
```toml
model_provider = "ShuYou"  # [!code highlight]
model = "openai/gpt-5.2-codex"  # [!code highlight]

[model_providers.ShuYou]  # [!code highlight]
name = "ShuYou"  # [!code highlight]
base_url = "https://api.shuyou.ai/v1"  # [!code highlight]
env_key = "ShuYou_API_KEY"  # [!code highlight]
wire_api = "responses"  # [!code highlight]
```

<Tip title="Configuration Notes">

- `model_provider`: Use ShuYou as the model provider
- `model`: The model to use; can be any model supported by ShuYou
- `base_url`: The base URL of the ShuYou API
...
You can freely switch the `model` field in `config.toml` to any model supported by ShuYou.

<Info title="Get the Model List">

- View models available under the Responses protocol via the [ShuYou model list](https://shuyou.ai/models?sort=newest&supported_protocol=responses)
...
For more models, see the [ShuYou model list](https://shuyou.ai/models?sort=newest&supported_protocol=responses).

### Responses-Compatible Models (Fetched on 2026-01-29)

The list below is fetched from the [ShuYou model list (Responses filter)](https://shuyou.ai/models?sort=newest&supported_protocol=responses). Use that link for the latest updates.
...
- Check whether the environment variable `ShuYou_API_KEY` is set correctly
- Use `echo $ShuYou_API_KEY` to verify the environment variable value
- Confirm the API Key is active and that you have sufficient balance
- Verify the API Key format starts with `sk-`


#### Connection Failure
**Issue**: Codex CLI cannot connect to the ShuYou service.

**Solutions**:

- Check that your network connection is working
- Verify that `base_url` in `config.toml` is correctly set to `https://api.shuyou.ai/v1`
- Confirm your firewall settings are not blocking outbound connections
- Try `curl https://api.shuyou.ai/v1/models` to test connectivity
...
- Confirm the environment variable is set correctly: `echo $ShuYou_API_KEY`
...
- Visit the [ShuYou model list](https://shuyou.ai/models?sort=newest&supported_protocol=responses) to confirm the model is available
