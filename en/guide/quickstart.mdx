...
Welcome to **[AGIPower](https://agipower.ai/about/intro.html)**! This guide walks you through everything from signing up to making your first API request, step by step.

<Tip title="💡 Get Started in 4 Steps">
Just four simple steps to start using AGIPower:
</Tip>

1. **Sign in to AGIPower**: Visit the **[AGIPower login page](https://agipower.ai/signin)** and sign in using any of the following:
...
Go to the **[Pay As You Go management page](https://agipower.ai/platform/pay-as-you-go)** to top up your balance and create an API key.

<img src="https://cdn.marmot-cloud.com/storage/agipower/2026/01/19/rsGoojf/pay-as-you-go.png" alt="Pay As You Go management page" />
...
Go to the **[Pricing page](https://agipower.ai/pricing/subscription)** to view plan details and subscribe.

<img src="https://cdn.marmot-cloud.com/storage/agipower/2026/01/28/iszJOqe/ss-1.png" alt="Subscription plan comparison" />
...
Each model on the AGIPower platform has a unique slug. You can find model slugs on the [Models page](https://agipower.ai/models):
![model-slug](https://cdn.marmot-cloud.com/storage/agipower/2025/10/21/AQG0SIr/model-slug.png)
Or on a [model's detail page](https://agipower.ai/google/gemini-3.1-pro-preview):
...
|----------|----------|---------------|-------------|
| **OpenAI Chat Completions** | `https://api.agipower.ai/v1` | OpenAI SDK | The most widely used Chat API |
| **OpenAI Responses** | `https://api.agipower.ai/v1` | OpenAI SDK | OpenAI's next-gen Responses API |
| **Anthropic Messages** | `https://agipower.ai` | Anthropic SDK | Native protocol for the Claude family |
| **Google Gemini** | `https://api.agipower.ai` | Google GenAI SDK | Native protocol for the Gemini family |
...
<Info title="Compatibility">
AGIPower fully supports the Anthropic Messages protocol and integrates seamlessly with tools like Claude Code and Cursor.

Note that the Anthropic protocol uses `base_url` `https://agipower.ai`, which differs from the OpenAI protocol.
</Info>

<Info title="Anthropic Protocol Model Support">
Models compatible with the Anthropic protocol are being adapted in batches. You can view currently supported models by filtering for Anthropic API Compatible on the [official model list](https://agipower.ai/models):
![anthropic-support](https://cdn.marmot-cloud.com/storage/agipower/2025/10/16/602FqX9/anthropic-support.png)
You can also check on a [model's detail page](https://agipower.ai/anthropic/claude-haiku-4.5):
...
client = Anthropic(
    # 2. Point the base URL to the AGIPower endpoint
    base_url="https://agipower.ai", # [!code highlight]
    # 3. Replace with the API key from your AGIPower console
    api_key="<your AGIPower_API_KEY>", # [!code highlight]
)
...
const client = new Anthropic({
  // 2. Point the base URL to the AGIPower endpoint
  baseURL: "https://agipower.ai", // [!code highlight]
  // 3. Replace with the API key from your AGIPower console
  apiKey: "<your AGIPower_API_KEY>", // [!code highlight]
});
...
<Info title="Compatibility">
AGIPower supports the Google Gemini (Vertex AI) protocol, allowing you to call models directly with the Google GenAI SDK.

The base URL is `https://api.agipower.ai`. You need to set `vertexai=True` and configure custom `http_options`.
</Info>
...
Platform APIs require a **Management API Key** for authentication (some endpoints also accept a regular API key). Go to **[AGIPower Console > Management](https://agipower.ai/platform/management)** to create a Management API Key.
</Warning>
