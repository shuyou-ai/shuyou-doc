...
head:
  ...
  - - meta
    - name: keywords
      content: AGIPower, guide, tutorial, reasoning, OpenAI, API
---

# Reasoning Models

AGIPower provides fine-grained control over model reasoning behavior and supports multiple API protocols:
...
AGIPower automatically computes and fills in the parameters required by the model based on what you provide, ensuring optimal reasoning behavior.
...
client = OpenAI(
    ...
    api_key=os.getenv("AGIPower_API_KEY"),
)
...
const openai = new OpenAI({
  ...
  apiKey: process.env.AGIPower_API_KEY,
});
