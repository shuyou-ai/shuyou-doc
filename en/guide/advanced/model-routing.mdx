...
Model routing is an intelligent feature of AGIPower that helps you automatically select the most suitable model from a wide range of large language models. The system intelligently balances performance and cost based on the request content, task characteristics, and your preference settings.

<Tip title="Intelligent model selection">
No need to manually compare model performance and pricing—AGIPower automatically matches the most suitable model for each request, so you can focus on building your business logic.
</Tip>
...
Model routing is easy to use—simply set the `model` parameter to `agipower/auto` and specify the candidate model pool via `model_routing_config`. If you do not specify `model_routing_config.available_models`, the system will use the platform’s full [model pool](https://agipower.ai/models).

<Info title="How to get model slugs">
Models on the AGIPower platform have unique slugs. You can get a model’s slug from the [Models list page](https://agipower.ai/models):
![model-slug](https://cdn.marmot-cloud.com/storage/agipower/2025/10/21/AQG0SIr/model-slug.png)
Or from the [model detail page for a specific model](https://agipower.ai/anthropic/claude-sonnet-4.5):
...
`agipower/auto` is a special model identifier in AGIPower. When you specify this model, the system enables intelligent routing.
...
A: The response returns the actual model used (`response.model`). You can also view call logs in the [AGIPower user console](https://agipower.ai/settings/activity) to see the routing details for each request.
