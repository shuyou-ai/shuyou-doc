...
head:
  ...
  - - meta
    - name: keywords
      content: ShuYou, guide, error codes, API, troubleshooting
---

# API Error Codes Reference

This document describes all possible error responses returned by ShuYou platform APIs, including root causes and troubleshooting steps.
...
2. Log in to the [ShuYou Console](https://shuyou.ai/platform) to confirm that the API Key has not been revoked or expired.
...
| `Model \{model\} is not valid` | Specified model identifier does not exist | Refer to the [model list](https://shuyou.ai/models) for valid model names |
...
1. Go to the [ShuYou Billing page](https://shuyou.ai/platform) to top up your balance.
...
1. Check the [Subscription Guide](https://shuyou.ai/docs/guide/subscription.html) to confirm which models are supported by your current plan.
...
2. Refer to the [model list](https://shuyou.ai/models) for valid model identifiers.
...
2. If the issue persists, contact support with the `X-ShuYou-RequestId` from the response headers.
...
3. **Save the `X-ShuYou-RequestId` from response headers** — Including this ID when contacting support helps expedite issue resolution.
...
A: Rate limits depend on your subscription plan tier. Log in to the [ShuYou Console](https://shuyou.ai/platform) to view your current plan details.

**Q: How can I tell which upstream provider handled my request?**

A: For security and abstraction purposes, provider information in responses is anonymized. If you need to trace the request path, provide the `X-ShuYou-RequestId` from the response headers to the support team.

**Q: What does `ShuYou PROCESSING` mean during a streaming request?**
