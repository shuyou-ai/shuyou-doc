...
head:
  ...
  - - meta
    - name: keywords
      content: AGIPower, guide, tutorial, structured, output, OpenAI, GPT, API
---

# Structured Outputs

AGIPower provides Structured Outputs to ensure model responses strictly follow the [JSON Schema](https://json-schema.org/) format you define.
When you have a fixed requirement for structured data, this feature is exactly what you need.

AGIPower supports structured outputs across multiple API protocols:
...
client = OpenAI(
    ...
    # 3. Replace with the API Key from the AGIPower user console
...
client = anthropic.Anthropic(
    # 2. Replace with the API Key from the AGIPower user console
    api_key="<your AGIPower_API_KEY>", # [!code highlight]
