...
AGIPower supports invoking image generation models via the Vertex AI protocol. This guide explains how to use AGIPower to generate images and save them locally.

<Tip title="💡 About Banana Models">
Banana is a series of image generation models from Google that can produce high-quality images from text prompts. You can use these models in AGIPower through the Vertex AI protocol.
</Tip>
...
Visit the [AGIPower model catalog](https://agipower.ai/models) to search and view all available image generation models.
</Tip>
...
const client = new genai.GoogleGenAI({
  apiKey: "$AGIPower_API_KEY",  // Replace with your API key
  vertexai: true,
  httpOptions: {
...
AGIPower supports two invocation modes:
