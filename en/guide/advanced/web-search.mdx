...
head:
  - - meta
    - name: description
      content: AGIPower Web Search Guide - Enable AI models to access real-time web information via Chat Completions, Messages, Responses, and Vertex AI protocols
  - - meta
    - name: keywords
      content: AGIPower, Web Search, API, OpenAI, Anthropic, Claude, GPT, Vertex AI, Google Search, real-time search, grounding
---

# Web Search

This document explains how to use the Web Search feature on the AGIPower platform. AGIPower supports invoking Web Search via multiple compatible protocols, including Chat Completions, Messages, Responses, and Vertex AI.
...
```cURL
curl -X POST "https://api.agipower.ai/v1/chat/completions" \
...
async function chatWithWebSearch() {
  const response = await client.chat.completions.create({
    ...
    // @ts-ignore - web_search_options is a AGIPower extension parameter
    web_search_options: {
      search_context_size: "medium",
      user_location: {
        type: "approximate",
...
```cURL
curl -X POST "https://api.agipower.ai/v1/messages" \
  -H "Content-Type: application/json" \
...
```cURL
curl -X POST "https://api.agipower.ai/v1/responses" \
  -H "Content-Type: application/json" \
  -H "Authorization: Bearer YOUR_API_KEY" \
  -d '{
    "model": "openai/gpt-5.2",
...
# Configure to use the AGIPower proxy
client = genai.Client(
    api_key="YOUR_API_KEY",
    vertexai=True,
