...
head:
  ...
  - - meta
    - name: keywords
      content: AGIPower, API, documentation, get, generation
---
...
- **参数名**：`Authorization`
- **格式**：`Bearer &lt;API_KEY&gt;`
- **说明**：你的 AGIPower API Key
  - **Pay As You Go API Key**：支持查询完整的计量和计费信息
  - **订阅制 API Key**（以 `sk-ss-v1-` 开头）：仅支持查询计量信息，不支持计费信息

<Tip title="💡 获取 API Key">
- Pay As You Go API Key：登录 [AGIPower 控制台](https://agipower.ai/platform/pay-as-you-go) 创建
- 订阅制 API Key：登录 [订阅管理页面](https://agipower.ai/platform/subscription) 创建
</Tip>
...
AGIPower API 接口返回的 generation id，可以从以下接口获得：
