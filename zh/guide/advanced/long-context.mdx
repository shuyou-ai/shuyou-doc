...
Anthropic Claude 系列模型支持将上下文窗口从默认的 200K token 扩展至 **1,000,000 token（1M）**，这是默认容量的 5 倍。通过 AGIPower，你可以轻松开启这一能力，处理超大规模的文档分析、代码审查、长对话等场景。

<Tip title="💡 核心优势">

- **超大上下文**: 单次请求可处理约 750,000 个英文单词或数千页文档
- **深度分析**: 适用于大规模代码库审查、长篇文献分析、完整对话历史等场景
- **无缝集成**: 通过 AGIPower 统一接口调用，只需添加一个请求头即可开启
</Tip>
