...
head:
  - - meta
    - name: description
      content: AGIPower 网络搜索功能指南 - 通过 Chat Completions、Messages、Responses、Vertex AI 等协议调用 Web Search，让 AI 模型访问实时网络信息
  - - meta
    - name: keywords
      content: AGIPower, Web Search, 网络搜索, API, OpenAI, Anthropic, Claude, GPT, Vertex AI, Google Search, 实时搜索
---

# 网络搜索

本文档介绍如何在 AGIPower 平台中使用 Web Search 功能。AGIPower 支持通过多种兼容协议调用 Web Search，包括 Chat Completions、Messages、Responses 以及 Vertex AI。
...
async function chatWithWebSearch() {
  const response = await client.chat.completions.create({
    ...
    // @ts-ignore - web_search_options 是 AGIPower 扩展参数
    web_search_options: {
      search_context_size: "medium",
      user_location: {
        type: "approximate",
