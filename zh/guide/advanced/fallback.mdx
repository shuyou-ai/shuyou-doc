...
模型兜底（Fallback）是 ShuYou 的容错保障机制，当主要模型或路由策略失败时，自动切换到备用模型继续处理请求。这确保您的应用始终能够获得响应，最大程度降低服务中断的风险。
...
ShuYou 支持在控制台后台设置全局的兜底模型，所有请求在失败时自动使用该模型作为备用方案。

### 配置方式

1. 访问 [ShuYou 策略设置页面](https://shuyou.ai/settings/strategy)
2. 找到 **Default Fallback Model** 配置项
3. 从下拉列表中选择您的全局兜底模型
4. 保存设置后立即生效

![设置全局兜底模型](https://cdn.marmot-cloud.com/storage/ShuYou/2025/10/21/r4HTlzi/fall-back.png)
...
A: 响应中的 `model` 字段会显示实际使用的模型。如果与请求中指定的模型不同，说明触发了兜底机制。您也可以在 [ShuYou 控制台](https://shuyou.ai/settings/activity) 查看详细的请求日志。
