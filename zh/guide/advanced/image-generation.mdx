...
ShuYou 支持通过 Vertex AI 协议调用图片生成模型。本指南将介绍如何使用 ShuYou 生成图片并保存到本地。

<Tip title="💡 关于 Banana 模型">
Banana 是 Google 推出的图片生成模型系列,能够根据文本描述生成高质量图片。您可以通过 Vertex AI 协议在 ShuYou 中使用这些模型。
</Tip>
...
<Tip title="📚 更多模型">
访问 [ShuYou 模型列表](https://shuyou.ai/models) 搜索查看所有可用的图片生成模型。
</Tip>

## API 参考

图像生成 API 及交互式 Playground：

- [GPT Image 2 图像生成](/zh/api-reference/image-series/openai/gpt-image-2-generate) — `POST /v1/predictions`
- [Gemini 3.1 Flash Image Preview 图像生成（Nano Banana 2）](/zh/api-reference/image-series/gemini/gemini-3.1-flash-image-preview-generate) — `POST /v1/predictions`
- [Gemini 3 Pro Image Preview 图像生成（Nano Banana Pro）](/zh/api-reference/image-series/gemini/gemini-3-pro-image-preview-generate) — `POST /v1/predictions`
- [Gemini 2.5 Flash Image 图像生成（Nano Banana）](/zh/api-reference/image-series/gemini/gemini-2.5-flash-image-generate) — `POST /v1/predictions`
...
const client = new genai.GoogleGenAI({
  apiKey: "$ShuYou_API_KEY",  // 替换为你的 API Key
  vertexai: true,
  httpOptions: {
