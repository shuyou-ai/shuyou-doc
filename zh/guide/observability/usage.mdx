...
head:
  ...
  - - meta
    - name: keywords
      content: AGIPower, guide, tutorial, usage, OpenAI, API
---

# 用量分析（Usage Analytics）

AGIPower 提供了全面的使用量分析功能，帮助您实时监控和分析 API 调用的使用情况、服务提供商表现以及模型性能。通过使用分析功能，您可以深入了解 Token 消耗、API 请求量、响应速度等关键指标，从而优化应用效率并控制成本。
...
![usage](https://cdn.marmot-cloud.com/storage/agipower/2026/01/19/VKnjOtl/yongliangfenxi-gailan.png)
...
![usage](https://cdn.marmot-cloud.com/storage/agipower/2026/01/19/RaM5XU5/yongliangfenxi-Usage-by-Model.png)

- **按类型分布（Usage by Token Type）**  
  展示不同类型的Token使用分布情况。

![usage](https://cdn.marmot-cloud.com/storage/agipower/2026/01/19/F1SB1uu/yongliangfenxi-Usage-by-Token-Type.png)

- **按 API Key 分布（Usage by API Key）**  
  按不同 API 密钥维度展示 Token 和请求使用情况，适用于多用户或多项目场景下的用量隔离与审计。

![usage](https://cdn.marmot-cloud.com/storage/agipower/2026/01/19/vCxvX0T/yongliangfenxi-Usage-by-API-Key.png)

- **按Web搜索分布（Web Search Usage）**  
  显示启用 Web 搜索功能的请求所消耗的 Token 量及调用次数（如适用），帮助评估增强检索功能的使用频率与开销。

![usage](https://cdn.marmot-cloud.com/storage/agipower/2026/01/19/sFwA29Z/yongliangfenxi-Web-Search-Usage.png)

## 服务提供商视图（Provider）

切换至“Provider”标签页，可查看不同 AI 服务提供商的使用表现。

![usage](https://cdn.marmot-cloud.com/storage/agipower/2026/01/19/Pbg1WCN/yongliangfenxi-Provider.png)
...
![usage](https://cdn.marmot-cloud.com/storage/agipower/2026/01/19/hz0tFzY/yongliangfenxi-Provider-Token-Distribution.png)

- **Request Distribution**  
  按服务提供商统计 API 请求次数分布，反映各提供商的调用负载情况。

![usage](https://cdn.marmot-cloud.com/storage/agipower/2026/01/19/DyqPfmR/yongliangfenxi-Provider-Request-Distribution.png)
...
![usage](https://cdn.marmot-cloud.com/storage/agipower/2026/01/19/9Y5ytNg/yongliangfenxi-Performance-gailan.png)
