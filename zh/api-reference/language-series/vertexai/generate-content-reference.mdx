...
示例：`POST https://api.shuyou.ai/v1beta/models/gemini-3.5-flash:generateContent`

ShuYou 支持 Google Gemini Native API 格式。具体请求参数和返回结构详见 [Google Vertex AI 官网](https://cloud.google.com/vertex-ai/generative-ai/docs/model-reference/inference)。

## Path parameters

### model `string` **必填**

Google Native API 格式的模型 ID（如 `gemini-3.5-flash`、`gemini-2.5-flash`）。
