...
head:
  - - meta
    - name: description
      content: 通过 ShuYou 使用 Gemini CLI 指南
  - - meta
    - name: keywords
      content: ShuYou, best practices, integration, Gemini CLI, Google, AI, terminal, agent
---

# 通过 ShuYou 使用 Gemini CLI 指南

Gemini CLI 是 Google 推出的开源 AI 终端代理工具，可以将 Gemini 的能力直接带入您的终端。它基于 ReAct（Reason + Act）循环架构，能够自主完成编程、调试、文件操作、内容生成等复杂任务。Gemini CLI 提供了 100 万 Token 上下文窗口、60 次/分钟的请求速率，以及丰富的 MCP 工具集成能力。通过 ShuYou，您可以为 Gemini CLI 配置自定义 API 端点，获得更灵活的模型选择和更稳定的服务体验。

## 前置条件

- Node.js 18+ 环境
- ShuYou API Key（参见下方[获取 ShuYou API Key](#获取-ShuYou-api-key)）
...
获取方式：
1. 访问 https://shuyou.ai/platform/subscription
2. 选择合适套餐
3. 在控制台创建订阅 API Key
```
...
获取方式：
1. 访问 https://shuyou.ai/platform/pay-as-you-go
2. 充值账户
...
<Warning title="重要配置">
请确保将 `sk-ss-v1-xxx` 替换为您的真实 ShuYou API Key。您可以在 [ShuYou 控制台](https://shuyou.ai/settings/keys) 中获取 API Key。
</Warning>
...
- `GEMINI_API_KEY`：您的 ShuYou API Key
- `GEMINI_MODEL`：指定使用的模型，可以是 ShuYou 支持的 Google 模型
- `GOOGLE_GEMINI_BASE_URL`：ShuYou Vertex AI 兼容端点
...
如果能正确读取文件并返回摘要，说明 ShuYou + Gemini CLI 的只读分析链路可用。
...
**通过 ShuYou 使用时**，选择 API Key 方式，将 ShuYou API Key 设置到 `GEMINI_API_KEY` 环境变量即可。
...
通过 ShuYou，您可以在 Gemini CLI 中使用多种 Gemini 模型。可以通过 `GEMINI_MODEL` 环境变量指定模型：
...
- 通过 [ShuYou 模型列表](https://shuyou.ai/models?sort=newest&supported_protocol=google) 查看 Google AI 协议可用模型
- 使用模型的 slug 名称（如 `google/gemini-2.5-pro`）
- 如需指定特定供应商，请参考 [Provider Routing 文档](/zh/guide/advanced/provider-routing)
</Info>

<Tip title="切换模型">
即使模型名可以切换成功，也不代表工具调用会一并可用。当前限制主要来自 ShuYou 对 Gemini CLI Agent 协议的兼容性，而不只是模型本身。
</Tip>
...
- 后续 ShuYou 补齐 function calling 兼容性后将自动支持，届时无需此修改
</Tip>
...
- 在 [ShuYou 控制台](https://shuyou.ai/settings/keys) 中验证 Key 状态


#### 连接失败问题
**问题**：Gemini CLI 无法连接到 ShuYou 服务

**解决方案**：

- 检查网络连接是否正常
- 验证 Base URL 是否配置正确为 `https://api.shuyou.ai`
- 确认防火墙设置是否阻止了外部连接
- 尝试使用 `curl https://api.shuyou.ai/v1/models` 测试连接
...
- 后续 ShuYou 补齐 function calling 兼容性后将自动支持
...
- 访问 [ShuYou 模型列表](https://shuyou.ai/models?sort=newest&supported_protocol=google) 确认模型是否可用
