...
head:
  - - meta
    - name: description
      content: 通过 AGIPower 使用 Claude Code CLI 指南
  - - meta
    - name: keywords
      content: AGIPower, best practices, integration, claude, code, Anthropic, Claude, GPT, API
---

# 通过 AGIPower 使用 Claude Code CLI 指南

<Info title="兼容性说明">

Claude Code 是 Anthropic 推出的官方 Coding Agent，通过与 AGIPower 的集成，您可以使用更多模型选择，而不仅仅局限于 Claude 官方模型。

例如，您可以通过 AGIPower 在 Claude Code 中使用 GPT-5.2系列、Claude-4.5系列、Gemini-3系列、Grok 4.1系列、Doubao-Seed-Code、Kimi-K2、Minimax-M2、GLM-4.6、DeepSeek-V3.2、Qwen3-Coder-Plus等模型，更多支持的模型请见[官方模型列表](https://agipower.ai/models?sort=newest&supported_protocol=messages)。

AGIPower 完全支持 Anthropic API 协议，可以无缝集成到 Claude Code、Cursor 等工具中。只需修改两个参数即可使用。

注意 Anthropic 协议的 `base_url="https://api.agipower.ai"`。
</Info>
...
Claude Code 默认直接连接到 Anthropic 官方服务，但通过配置环境变量，我们可以将其请求重定向到 AGIPower 服务。这样做的好处是：

- **无需修改 Claude Code 本身**：仅通过环境变量即可切换服务端点
- **使用 AGIPower API Key 认证**：替代 Anthropic 官方 API Key
- **访问更多模型选择**：除 Claude 系列外，还可使用 GPT、Gemini、Qwen 等多种模型

配置的核心是设置两个关键环境变量：`ANTHROPIC_BASE_URL`（AGIPower 服务地址）和 `ANTHROPIC_AUTH_TOKEN`（您的 AGIPower API Key），从而让 Claude Code 的所有请求都通过 AGIPower 转发。
...
因此，连接 AGIPower 时建议统一使用 **shell profile 环境变量** 的方式进行配置，以确保登录与请求都走 AGIPower 的 Anthropic 兼容端点。
</Warning>

### 步骤 0：获取 AGIPower API Key

在配置 Claude Code 之前，您需要先获取 AGIPower API Key。AGIPower 提供两种计费方案，请根据使用场景选择：
...
本步骤将把 AGIPower 连接配置写入您的 shell 配置文件中，使其在每次打开终端时自动生效。
...
# ============= AGIPower + Claude Code 配置 =============
# 将 Claude Code 连接到 AGIPower 服务，而非 Anthropic 官方服务

# 核心配置：AGIPower 服务端点和认证
export ANTHROPIC_BASE_URL="https://api.agipower.ai"  # AGIPower Anthropic 兼容端点
export ANTHROPIC_AUTH_TOKEN="sk-ss-v1-xxx"                   # 替换为您的 AGIPower API Key（订阅制 sk-ss-v1-xxx 或按量付费 sk-xxx）
...
# 核心配置：AGIPower 服务端点和认证
$env:ANTHROPIC_BASE_URL = "https://api.agipower.ai"  # AGIPower Anthropic 兼容端点
$env:ANTHROPIC_AUTH_TOKEN = "sk-ss-v1-xxx"                   # 替换为您的 AGIPower API Key（订阅制 sk-ss-v1-xxx 或按量付费 sk-xxx）
...
请确保将配置中的 `sk-ss-v1-xxx` 或 `sk-xxx` 替换为您的真实 AGIPower API Key：
...
| 变量名                                     | 作用         | 说明                                      |
| ------------------------------------------ | ------------ | ----------------------------------------- |
| `ANTHROPIC_BASE_URL`                       | 服务端点地址 | 将 Claude Code 的请求重定向到 AGIPower 服务 |
| `ANTHROPIC_AUTH_TOKEN`                     | 认证密钥     | 您的 AGIPower API Key（订阅制或按量付费）   |
...
配置完环境变量后，就可以启动 Claude Code 了。首次启动时会自动通过 AGIPower 完成认证。
...
4. 首次启动时，Claude Code 会：
   ...
   - 通过 `ANTHROPIC_BASE_URL` 指向的 AGIPower 服务完成认证
   - 无需额外登录步骤，即可开始使用
...
启动成功后，建议验证 Claude Code 是否正确连接到 AGIPower 服务。
...
Anthropic base URL: https://api.agipower.ai  # [!code highlight]
```

**验证要点：**

- ✅ `Auth token` 应显示为 `ANTHROPIC_AUTH_TOKEN`（表示从环境变量读取）
- ✅ `Anthropic base URL` 应显示为 `https://api.agipower.ai`（AGIPower 服务地址）

如果显示的信息符合上述要求，说明配置成功！您现在可以通过 AGIPower 使用 Claude Code 了。
...
&lt;div style="text-align: center;"&gt;
  <img src="https://cdn.marmot-cloud.com/storage/agipower/2026/02/03/S4ThMGS/cc-vs.png" alt="安装 Claude Code Extension" />
&lt;/div&gt;
...
&lt;div style="text-align: center;"&gt;
  <img src="https://cdn.marmot-cloud.com/storage/agipower/2026/02/03/lDKDS2r/cc-vs2.png" alt="打开 Claude Code 设置" />
&lt;/div&gt;
...
&lt;div style="text-align: center;"&gt;
  <img src="https://cdn.marmot-cloud.com/storage/agipower/2026/02/03/w8EeQzh/cc-vs3.png" alt="配置模型和环境变量" />
&lt;/div&gt;
...
1. **替换 API Key**：将 `sk-ss-v1-xxx` 替换为您的真实 AGIPower API Key
...
配置完成后，您就可以在 Claude Code 中使用 AGIPower 的多种模型了：
...
#### 从其他平台切换到 AGIPower 时认证失败
**问题**：之前使用过 Claude Code 官方账号或其他平台（如 MiniMax、GLM 等），切换到 AGIPower 后出现认证失败或配置冲突

**解决方案**：

这通常是因为旧的配置文件缓存了之前的认证信息，导致与新的 AGIPower 配置冲突。按照以下步骤清理并重新配置：
...
2. **确认 shell 环境变量配置正确**：
   ...
   检查您的 shell 配置文件（`~/.zshrc` 或 `~/.bashrc`）中是否包含完整的 AGIPower 配置。参考本文档上方的 [步骤 1：配置 Shell 环境变量](#步骤-1-配置-shell-环境变量-推荐方式) 章节，确保包含以下关键环境变量：

   ```bash
   export ANTHROPIC_BASE_URL="https://api.agipower.ai"
   export ANTHROPIC_AUTH_TOKEN="sk-ss-v1-xxx"  # 替换为您的 AGIPower API Key
...
4. **验证环境变量已生效**：
   ...
   # 应输出：https://api.agipower.ai

   # 检查 API Key 是否已设置
   echo $ANTHROPIC_AUTH_TOKEN
   # 应输出您的 AGIPower API Key
...
6. **验证连接状态**：
   ...
   Anthropic base URL: https://api.agipower.ai
   ```
...
- 请通过 [AGIPower 模型列表](https://agipower.ai/models) 筛选 "Anthropic API Compatible" 查看当前支持的模型
- 或访问具体模型的详情页确认是否支持 Anthropic 协议
- 选择上述支持列表中的模型进行使用


#### 连接失败问题
**问题**：Claude Code 无法连接到 AGIPower 服务

**解决方案**：

- 检查网络连接是否正常
- 验证 `ANTHROPIC_BASE_URL` 是否配置正确为 `https://agipower.ai`
- 确认防火墙设置是否阻止了外部连接
...
2. **检查模型配置**：
   ...
   - 确认该模型支持 Anthropic 协议（通过[模型列表](https://agipower.ai/models?sort=newest&supported_protocol=messages)查看）
...
<Info title="更多模型">
查看 [AGIPower 模型列表](https://agipower.ai/models) 了解所有可用模型及其详细信息
</Info>
