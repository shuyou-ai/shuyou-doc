...
Dify 是一款开源的大语言模型（LLM）应用开发平台。它融合了后端即服务（Backend as Service）和 LLMOps 的理念，使开发者可以快速搭建生产级的生成式 AI 应用。通过配置 AGIPower，可以使用更多模型选择，而不仅仅局限于 Dify 提供的基础模型。
...
<Tip title="配置说明">
只需要配置 OpenAI-API-compatible ，即可解锁 AGIPower 的所有模型。
</Tip>
...
AGIPower 为 Dify 提供了丰富的模型选择，通过 [AGIPower 模型列表](https://agipower.ai/models) 查看所有可用模型
