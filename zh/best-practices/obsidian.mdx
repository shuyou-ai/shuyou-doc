...
head:
  - - meta
    - name: description
      content: 通过 AGIPower 使用 Obsidian Copilot 指南
  - - meta
    - name: keywords
      content: AGIPower, best practices, integration, obsidian, OpenAI, API
---

# 通过 AGIPower 使用 Obsidian Copilot 指南

Obsidian Copilot 是一个强大的 AI 助手插件,通过与 AGIPower 的集成,您可以在 Obsidian 笔记中使用多种先进的 AI 模型,获得更智能的写作和思考体验。

<Info title="兼容性说明">
AGIPower 完全支持 OpenAI API 协议,可以无缝集成到 Obsidian Copilot 插件中。只需简单配置即可使用。
...
<Warning title="重要配置">
在配置之前,您需要先获取 AGIPower API Key。请访问 [AGIPower 控制台](https://agipower.ai/settings/keys) 创建或查看您的 API Key。
</Warning>
...
- **Base URL**: 填写 `https://api.agipower.ai/v1`
- **API Key**: 填写您的 AGIPower API Key（以 `sk-` 开头）
- **Model ID**: 填写 AGIPower 上支持 OpenAI Chat-Completion 协议的模型 slug
...
- 在 [AGIPower 控制台](https://agipower.ai/settings/keys) 查看 API Key 状态


#### 连接失败问题
**问题**: Copilot 无法连接到 AGIPower 服务

**解决方案**:

- 检查网络连接是否正常
- 验证 Base URL 是否配置为 `https://api.agipower.ai/v1`（注意是 `/v1` 而不是 `/anthropic`）
- 确认防火墙或代理设置没有阻止连接
- 尝试在浏览器中访问 [https://agipower.ai](https://agipower.ai) 确认服务可访问
...
- 访问 [AGIPower 模型列表](https://agipower.ai/models) 筛选 "OpenAI API Compatible" 查看可用模型
